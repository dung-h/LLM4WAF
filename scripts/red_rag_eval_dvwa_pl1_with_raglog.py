...
def main():
    ...
    parser.add_argument("--num_samples", type=int, default=100, help="Number of samples to test for each mode.")
    ...
    parser.add_argument("--output_file", type=str, default="eval/red_rag_eval_dvwa_pl1_with_raglog.jsonl")
    parser.add_argument("--output_summary", type=str, default="eval/red_rag_eval_dvwa_pl1_with_raglog_summary.txt")
    args = parser.parse_args()

    cmd_str = f"python scripts/red_rag_eval_dvwa_pl1_with_raglog.py --num_samples {args.num_samples}"
    ...
    try:
        ...
        with httpx.Client(base_url=DVWA_WAF_URL, timeout=15.0) as client:
            ...
            print(f"\n--- Running RED in 'RAG_V2' mode (with RAG logs) ---")
            ...
            # Tạo danh sách test_cases đầy đủ để lặp
            full_test_cases_iterable = (test_cases * (args.num_samples // len(test_cases)))
            full_test_cases_iterable.extend(test_cases[:args.num_samples % len(test_cases)])

            for i, case in enumerate(tqdm(full_test_cases_iterable, total=args.num_samples)):
                prompt_text, rag_docs_used = build_red_prompt_with_rag( # Corrected access
                    attack_type=case['attack_type'],
                    target_desc=case['target_desc'],
                    signals=case['signals'],
                    history_payloads=history_payloads, # Pass simplified history
                    corpus_version="v2" # Ensure it uses the v2 corpus
                )
                ...
                results.append({
                    "mode": "rag_v2",
                    "attack_type": case['attack_type'],
                    "waf_profile": "modsec_pl1", # Fixed for this eval
                    "target": case['target_desc'],
                    "payload": payload,
                    "result": status,
                    "rag_docs_used": rag_docs_used # Log RAG docs
                })
            summary["rag_v2"] = mode_results
            print(f"  RAG_V2 results: Total={mode_results['total']}, Passed={mode_results['passed']}, Blocked={mode_results['blocked']}")
        ...
        summary_content = []
        summary_content.append("--- RED RAG Extended Evaluation Summary (DVWA PL1, with RAG logs) ---")
        mode = "rag_v2" # Only rag_v2 mode for this script
        data = summary[mode]
        ...
        print(f"Error running extended eval: {e}", file=sys.stderr)
