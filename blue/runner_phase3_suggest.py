...
SUGGESTIONS_OUTPUT_FILE = "data/blue/blue_phase3_suggestions.jsonl"
MAX_EPISODES_TO_PROCESS = 200 # Limit to avoid excessive processing with stub LLM
...
def main():
    ...
    print(f"Selected {len(target_episodes)} false negative episodes for Phase 3 processing.")
    ...
    for i, episode in enumerate(target_episodes):
        ...
        print(f"  Generated recommendation for episode {i+1}. Vuln Effect: {llm_output['vuln_effect']}")


    # 4. Save suggestions
    with open(SUGGESTIONS_OUTPUT_FILE, 'w', encoding='utf-8') as f_out:
        for s in suggestions:
            f_out.write(json.dumps(s) + "\n")
    print(f"\nSaved {len(suggestions)} suggestions to {SUGGESTIONS_OUTPUT_FILE}")
